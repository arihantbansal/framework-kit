...
export function useSplToken(
    ...
    const helperConfig = useMemo<SplTokenHelperConfig>(
        ...
        [
            normalizedMint,
            options.commitment,
            options.config?.associatedTokenProgram,
            options.config?.decimals,
            options.config?.tokenProgram,
        ],
    );
    ...
    const { data, error, isLoading, isValidating, mutate } = useSWR<SplTokenBalanceResult>(
        balanceKey,
        () => helper.fetchBalance(owner!, options.commitment),
        {
...
export function useTransactionPool(config: UseTransactionPoolConfig = {}): Readonly<{
    ...
    const initialInstructions = useMemo<TransactionInstructionList>(() => config.instructions ?? [], [config.instructions]);
    const client = useSolanaClient();
